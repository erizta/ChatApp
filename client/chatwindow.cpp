...
void ChatWindow::chatPage()
{
    ...
    ui->unreadmessage->setText("0");
}

void ChatWindow::sendMessage()
{
    QString message = _sendMessage + _tcpSeparator + ui->Recipient->text() + _tcpSeparator + ui->msg->text();
    if (sendData(message))
    {
        ui->discussion->append("Me -> " + ui->Recipient->text() + ":  " + ui->msg->text());
        ui->msg->clear();
    }
    else
    {
        // TODO: display why the message wasn't sent
        ui->discussion->append(QString::fromLatin1("Me: Messages not sent"));
    }
    ui->msg->setFocus();
}
...
void ChatWindow::readData(const QString& receivdMessage)
{
    ...
    else if (result[1] == _newMessage)
    {
        ...
        ui->unreadmessage->setPalette(p);
        ui->unreadmessage->setText(result[2]);
    }
...
void ChatWindow::changeDestination(QListWidgetItem *item)
{
    ...
    if (! item)
        return;
    ui->Recipient->setText(item->text());
}
 ...
 void ChatWindow::reuseMessage()
{
    ...
    ui->Recipient->setText(selectedItem.at(2)->text());
...
{
    QMessageBox::information(this, "AAA CLIENT", error);
}
...
void ChatWindow::drawInterface()
{
    ...
    // tampilkan username
    ui->username->setText("<FONT COLOR=#EBDDE2>" + m_userName + "</FONT>");
    ui->username->setFont(QFont("Comic Sans MS", 18, QFont::Bold, true));
    ui->Lnew_messages->setStyleSheet("QLabel { color : white; }");
    ui->Lnew_messages->setAlignment(Qt::AlignRight);
    ui->unreadmessage->setStyleSheet("QLabel { color : white; }");
    ui->unreadmessage->setAlignment(Qt::AlignRight);
    ui->msgLabel->setStyleSheet("QLabel { color : white; }");
    ui->desLabel->setStyleSheet("QLabel { color : white; }");

    // gambar user
    if (QFile::exists("user.png"))
        ui->userimage->setIcon(QIcon("user.png"));
    else
        ui->userimage->setIcon(QIcon(":/img/user_profile.png"));
    ...
    connect(ui->actionSignout, SIGNAL(triggered()), this, SLOT(disconnect()));
    connect(ui->actionChat, SIGNAL(triggered()), this, SLOT(chatPage()));
    connect(ui->actionMessages_received, SIGNAL(triggered()), this, SLOT(ShowReceivedMessages()));
    connect(ui->actionMessages_sent, SIGNAL(triggered()), this, SLOT(ShowSentMessages()));
    ...
    connect(ui->actionChanger_password,SIGNAL(triggered()),this,SLOT(changePassword()));
    connect(ui->remove,SIGNAL(clicked()),this,SLOT(DeleteMessage()));
