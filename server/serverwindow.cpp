...
ServerWindow::ServerWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    if (!m_serveur->listen(QHostAddress::Any, 6606))
    {
        ui->L_serverStatus->setText(QString::fromUtf8("The server could not be started. Reason :<br />") +
                                    m_serveur->errorString());
    }
    else
    {
        ui->L_serverStatus->setText(QString::fromUtf8("The server was started on the port <strong>") +
                                    QString::number(m_serveur->serverPort()) +
                                    "</strong>.<br />Clients can now login.");
...
{
    // 1 : menerima paket (atau subpacket) dari salah satu klien

    // On détermine quel client envoie le message (recherche du QTcpSocket du client)
    QTcpSocket* socket = qobject_cast<QTcpSocket*>(sender());
    if (socket == 0)  // Jika tidak menemukan klien di asal sinyal, menghentikan metode
        return;
...
void ServerWindow::DrawInterface()
{
    ...
    ui->L_clientsNumber->setText(QString::fromUtf8("No connected clients"));
    ui->L_tcpNumber->setText(QString::fromLatin1("0 connected Tcp"));
